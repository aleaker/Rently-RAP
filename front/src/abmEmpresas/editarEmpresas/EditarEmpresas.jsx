...
class EditarEmpresas extends React.Component{
    ...
    componentDidMount(){
      ...
      axios.post('/api/createCompany/get/', {id: enterprise})
      .then(e=>{console.log(e.data)
                company = e.data
                    return company})
      .then(e=>{Object.entries(e.Company).map(obj=>{
        let objKey = obj[0]
        let objValue= obj[1]
        let newObj = {}
    ...
    updateBusiness(){
      ...
      axios.post('/api/createCompany/update', {userId: this.state.userId, id: enterprise, users: UsersSchema, Company, Commission: CommissionSchema})
  }
    ...
    render(){
        ...
        <h5 >Datos Empresa</h5>
        <Button onClick={()=> this.handleDisable({disableEmpresa:!this.state.disableEmpresa})}>
          
        {this.state.disableEmpresa?'Editar':'Guardar'}</Button>
        </div>
        ...
        <div style={{display: 'inline-flex', justifyContent: 'space-between', width: '100%', alignItems:'flex-end'}}> 
        <h5 >Ubicacion</h5>
        <Button onClick={()=> this.handleDisable({disableUbicacion:!this.state.disableUbicacion})}>
          
        {this.state.disableUbicacion?'Editar':'Guardar'}</Button>
        </div>
          <Row>
            <Col xs={12} md={4}>
              <label>Pais</label>
              <input
                ...
                placeholder="Pais"
                id="business_Country"
                onChange={evt => this.handleChange({ Country: evt.target.value })}
        ...
        <div style={{display: 'inline-flex', justifyContent: 'space-between', width: '100%', alignItems:'flex-end'}}> 
        <h5 >Contacto</h5>
        <Button onClick={()=> this.handleDisable({disableContacto:!this.state.disableContacto})}>
          
        {this.state.disableContacto?'Editar':'Guardar'}</Button>
        </div>
          <Row>
            <Col xs={6} md={3}>
              ...
              <input
                ...
                id="business_Description"
                onChange={evt =>
                  this.handleChange({ internationalCountryCode: evt.target.value })
                }
        ...
        <Button onClick={()=> this.handleDisable({disableDatosBancarios: !this.state.disableDatosBancarios})}>
          
        {this.state.disableDatosBancarios?'Editar':'Guardar'}</Button>
        </div>
          <Row>
            <Col xs={6}>
              <label>Banco</label>
